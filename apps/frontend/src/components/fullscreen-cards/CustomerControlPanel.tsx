      ...
      <AnimatePresence mode="wait">
        {!isExpanded ? (
          <motion.div
            ...
            className="w-full max-w-3xl rounded-2xl border border-white/10 bg-[#222221] p-4 lg:p-6 xl:p-8 shadow-2xl max-h-[90vh] overflow-y-auto"
          >
            {/* Header */}
            ...
            {/* Alignment Range */}
            <div className="mb-3 lg:mb-5 rounded-xl border border-white/10 bg-white/5 p-3 lg:p-4 xl:p-6">
            ...
            <div className="mb-3 lg:mb-5">
              ...
              <div className="flex gap-1 lg:gap-1.5 xl:gap-2">
                {customers.slice(0, 8).map((customer) => (
                  <div
                    key={customer.id}
                    className={`flex h-9 w-9 lg:h-10 lg:w-10 xl:h-12 xl:w-12 items-center justify-center rounded-full border-2 border-white/20 text-xs lg:text-sm font-bold text-white transition-transform hover:scale-110 hover:border-emerald-400/50 ${getColorFromName(customer.name)}`}
                  >
                    {getInitials(customer.name)}
                  </div>
                ))}
                {customers.length > 8 && (
                  <div className="flex h-9 w-9 lg:h-10 lg:w-10 xl:h-12 xl:w-12 items-center justify-center rounded-full border-2 border-dashed border-white/20 text-xs font-medium text-slate-400">
                    +{customers.length - 8}
                  </div>
                )}
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 lg:gap-3">
              <button
                ...
                className="group relative flex-1 overflow-hidden rounded-xl border border-white/10 py-2.5 lg:py-3 xl:py-4 text-sm lg:text-base font-medium text-white transition-all hover:text-white"
              >
                <div
